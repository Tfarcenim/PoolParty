...
public class FloatMatItem extends Item implements Colorable{

    private static final Predicate<Entity> ENTITY_PREDICATE = EntitySelector.NO_SPECTATORS.and(Entity::isPickable);
    private final DyeColor color;


    public FloatMatItem(Properties properties, DyeColor color) {
        super(properties);
        this.color = color;
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (hitresult.getType() == HitResult.Type.MISS) {
            ...
            if (hitresult.getType() == HitResult.Type.BLOCK) {
                FloatMatEntity tube = this.getTube(level, hitresult,itemstack);
    ...
    private FloatMatEntity getTube(Level level, HitResult hitResult, ItemStack stack) {
        return new FloatMatEntity(level, hitResult.getLocation().x, hitResult.getLocation().y, hitResult.getLocation().z,stack);
    }
}
